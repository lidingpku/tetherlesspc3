...
public class LoadAppLogic {
	...
	public static String getDBEntry (DatabaseEntry DBEntry,
		...
		if ("P2Detection".equalsIgnoreCase(FileEntry.TargetTable))
			SqlStr = String.format("SELECT detectID FROM %1$s LIMIT1", FileEntry.TargetTable);
		else if("P2ImageMeta".equalsIgnoreCase(FileEntry.TargetTable))
			SqlStr = String.format("SELECT imageID FROM %1$s LIMIT1", FileEntry.TargetTable);
		else
			SqlStr = String.format("SELECT frameID FROM %1$s LIMIT1", FileEntry.TargetTable);
